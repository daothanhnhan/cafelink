  ...
  //  //          }
	...
	/*----------------- Trang nội dung -------------------*/
		...
		public function deletePage(){
			...
			if ($page_id == 37 || $page_id == 38 || $page_id == 39) {
...
/*---------------- Danh mục dịch vụ --------------------*/
	...
	/*------------------ dịch vụ -------------------*/
		...
		public function updateLinhVuc(){
	    	...
	    	$array_child = $this->getListChild('linh_vuc','productcat_parent', $productcat_id, 'productcat_id', 'productcat_id', 'desc');
	    	...
	    	$friendly_url = $this->GetFinalSlug($friendly_url, 'linh_vuc', $productcat_id);

	    	$lang_showMain = "vn";	

	    	$this->Update_1('linh_vuc_languages', 
		...
		public function addLinhVuc(){
			// echo json_encode(array('error'=>'Lỗi, mời thử lại linh vuc.'));die;
			$productcat_name = isset($_POST['productcat_name']) ? $_POST['productcat_name'] : '';
		    ...
		    $friendly_url = $this->GetFinalSlug($friendly_url, 'linh_vuc', "");



	    	$insert = $this->Insert_1('linh_vuc', 
	    	...
	    	if($insert != ''){
	    		...
	    		foreach ($languages as $key => $lang) {
	    			...
	    			$insert1 = $this->Insert_1('linh_vuc_languages', 
	    		...
	    		echo json_encode(array('success'=>'Thêm mới thành công', 'url'=>'index.php?page=sua-danh-muc-linh-vuc&id='.$insert));
		...
		public function deleteLinhVuc(){
			...
			$array_child = $this->getList('linh_vuc','productcat_parent', $productcat_id, 'productcat_id', 'desc','','','');

			if ($array_child) {

				$current_parent = $this->getDetail_New('linh_vuc',array('productcat_id'),array(&$productcat_id),'i');

				$current_parent = $current_parent['productcat_parent'];

				foreach ($array_child as $key => $val) {

					$this->Update_1('linh_vuc',array('productcat_parent'),array(&$current_parent, &$val['productcat_id']),array('productcat_id'),'ii');
			...
			if($this->Delete_1('linh_vuc','',array(&$productcat_id),array('productcat_id'),'i')){

				echo json_encode(array('success'=>'Xóa thành công','url'=>'index.php?page=danh-muc-linh-vuc'));
		...
		public function addProduct(){
			...
			$linh_vuc_web_id = isset($_POST['linh_vuc_web_id']) ? $_POST['linh_vuc_web_id'] : '0';

			$loai_web_id = isset($_POST['loai_web_id']) ? $_POST['loai_web_id'] : '0';

			$hoan_thien_web_id = isset($_POST['hoan_thien_web_id']) ? $_POST['hoan_thien_web_id'] : '0';
			...
			if ($productCats_ar) {
				...
				echo json_encode(array('error'=>'Mời bạn chọn dạng web.'));die;
			}
			/*************************************/
			$linhvucCats_ar = isset($_POST['linhvucCats']) ? $_POST['linhvucCats'] : '';
			if ($linhvucCats_ar) {
				$linhvucCats_ar[count($linhvucCats_ar)] = $_POST['linhvucCats'][count($_POST['linhvucCats'])-1];
			} else {
				echo json_encode(array('error'=>'Mời bạn chọn lĩnh vực web.'));die;
			}
			// $product_cat = array();
			// /*-----------------------------------*/
			// if ($productCats_ar) {
   ...
   //          }
			...
			$product_cat = implode(",",$productCats_ar);
			$linhvuc_cat = implode(",",$linhvucCats_ar);
			/*************************************/
	  ...
	  //   	if(isset($_FILES['fileUpload2']) && $_FILES['fileUpload2']['name'] != ""){
			...
			$insert = $this->Insert_1('product', 
									   ...
									   'sssssssdddsssisiiissssssssssssssssiiiissssiiissi');
