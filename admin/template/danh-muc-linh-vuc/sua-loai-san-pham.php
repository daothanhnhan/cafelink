...
<?php
    ...
    $row = $action->getDetail_New('linh_vuc', array('productcat_id'), array(&$productcat_id), 'i');
    if ($row == '') {
        header('location: index.php?page=sua-danh-muc-linh-vuc');
    }
    $list = $action->getList('linh_vuc','','','productcat_id','desc','','','');
    $languages = $action->getListLanguages();

    $action_showMain = new action_page('VN');
    $lang_showMain = "vn";
    $row_showMain = $action_showMain->getDetail_New('linh_vuc_languages',array('productcat_id','languages_code'),array(&$row['productcat_id'], &$lang_showMain),'is');
...
<form action="" method="post" accept-charset="utf-8" id="updateLinhVuc">
    ...
    <input type="hidden" name="action" value="updateLinhVuc">
    ...
    <button type="button" class="btn btnDelete" id="deleteLinhVuc" data-id="<?= $productcat_id?>" data-action="deleteLinhVuc" <?php echo ($_SESSION['admin_role']==2)?'style="display: none;"':'';?> >Xóa</button>
</form>
