<?php 
    ...
    if (isset($_POST['del_video'])) {
        $id = $_GET['id'];
        $sql = "DELETE FROM hoan_thien_web WHERE id = $id";
        $result = mysqli_query($conn_vn, $sql);
        header('location: index.php?page=hoan-thien-web');
    }
    ...
    function editVideo ($id) {
        ...
        if (isset($_POST['edit_video'])) {
            ...
            if ($image == '') {
                $sql = "UPDATE hoan_thien_web SET name = '$name', des = ?, content = ?, title = '$title', meta_des = '$meta_des', meta_key = '$meta_key', url = '$url' WHERE id = $id";
            } else {
                $sql = "UPDATE hoan_thien_web SET name = '$name', des = ?, content = ?, title = '$title', meta_des = '$meta_des', meta_key = '$meta_key', url = '$url', image = '$image' WHERE id = $id";
            }
            // echo $sql;
            $stmt = $conn_vn->prepare($sql);
            $stmt->bind_param("ss", $des, $content);
            $stmt->execute();
            echo '<script>alert(\'Bạn đã sửa mức độ hoàn thiện web thành công.\')</script>';
        }
    }
    editVideo($_GET['id']);
    ...
    function getVideo ($id) {
        global $conn_vn;
        $sql = "SELECT * FROM hoan_thien_web WHERE id = $id";
        $result = mysqli_query($conn_vn, $sql);
