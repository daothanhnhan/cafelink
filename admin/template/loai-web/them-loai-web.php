<?php 
    ...
    function addVideo () {
        ...
        if (isset($_POST['add_video'])) {
            ...
            $sql = "INSERT INTO loai_web (name, image, des, content, title, meta_des, meta_key, url) VALUES ('$name', '$image', ?, ?, '$title', '$meta_des', '$meta_key', '$url')";
            $stmt = $conn_vn->prepare($sql);
            $stmt->bind_param("ss", $des, $content);
            $stmt->execute();
            $id = $stmt->insert_id;
            echo '<script>alert(\'Bạn đã tạo loại web thành công.\');window.location.href="index.php?page=sua-loai-web&id='.$id.'";</script>';
        }
    }
    addVideo();
?>
