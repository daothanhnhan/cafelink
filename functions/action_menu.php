...
class action_menu extends library{
    ...
    /*-----------------------------*/
     ...
     /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuLDP($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
                ...
                if($level == 0 && $row[$this->nameColTitle_menuLanguage] == "Trang chủ")
                  echo '<li>'; 
                    ...
                    if($level == 0 && $row[$this->nameColTitle_menuLanguage] != "Trang chủ")
                        ...
                        echo '<li class="has-sub">'; 
                 echo '<a href="'.$url.'">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    $this->showMenu_byMultiLevel_mainMenuLDP($list_menu,$item[$this->nameColID_menu],$lang, $level+1);                
                echo '</li>';            
            }
            echo '</ul>';        
        } 
    ...
    /*-----------------------------*/
     ...
     /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuLD_footer($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {    
            ...
            foreach ($cate_child as $key => $item) {
                ...
                echo '<li>';
                echo '<a href="'.$url.'"><span>+</span>  '.$row[$this->nameColTitle_menuLanguage].'</a>';
                echo '</li>';            
            }
            echo '</ul>';        
        } 
